...
$get_gri_forwardto = "";
	...
	$result2 = mysqli_query($conn,"SELECT COUNT(site) as total_balance FROM pass WHERE site='$get_site'");
	$Allsurvey = mysqli_fetch_array($result2);
	$get_id = $Allsurvey["total_balance"] ;
	//$ticke_id = $get_field_5.'-'.($get_id + 1);
	$ticke_id = rand(00000,99999);
	
	
	
	$result = mysqli_query($conn,"INSERT INTO pass
