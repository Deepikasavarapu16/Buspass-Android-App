...
/****************************************************************************/
	...
	{		
        $http.post('https://myapphosting.in/android/buspass/user_login.php', 
		...
		.success(function(data, status, headers, config) 
		{
			...
			else if(data.success == 3)
			{
				...
				window.location = "college_home.html";  // Home Page
				return;				
			}
			else if(data.success == 5) // College Head Login 
			{
				alert("Login Successful");
				$cookieStore.put("cook_user_mail",data.email);
				$cookieStore.put("cook_user_mob",data.mobile);
...
/****************************************************************************/
	...
	{		
        $http.post('https://myapphosting.in/android/buspass/admin_login.php', 
			{'email': $scope.email, 'password':$scope.password})
		.success(function(data, status, headers, config) 
		{
			if(data.success == 1)
...
//************************** admin_register **********************************/	
	...
	{		
		$http.post('https://myapphosting.in/android/buspass/admin_register.php',{
...
/************************** Update User Info **********************************/
		...
		$http.post('https://myapphosting.in/android/buspass/get_user_info.php',
		{
	...
	{		
		$http.post('https://myapphosting.in/android/buspass/admin_update.php',{
		 'name':$scope.name,'email':$scope.email,
		 'password':$scope.password,'mobile': $scope.mobile})
		.success(function(data, status, headers, config) 
		{
...
/****************************************************************************/
	...
	{		
        $http.post('https://myapphosting.in/android/buspass/newpassword.php', 
			{	'email': $scope.email, 'password':$scope.password,
